...
const vec3 fogNightColor = vec3(0.02, 0.05, 0.1);
...
void main() {
	...
	vec3 linearFogColor = pow(fogDayColor, vec3(2.2));

	if (depth == 1.0) {
		color.rgb = linearFogColor;
		return;
	}
	...
	float shadow = 1.0;
	if (shadowScreenPos.x > 0.0 && shadowScreenPos.x < 1.0 &&3
